        ...
        <div class="grid gap-1.5 border-b p-4 last:border-none dark:border-gray-800">
            ...
            <p class="flex items-center gap-2 text-gray-600 dark:text-gray-300">
                @if ($note->customer_notified)
                    ...
                    @lang('admin::app.customers.customers.view.notes.customer-notified', ['date' => core()->formatDate($note->created_at, 'm/d/Y h:i:s A')])
                @else
                    <span class="icon-cancel-1 h-fit rounded-full bg-red-100 text-2xl text-red-600"></span>

                    @lang('admin::app.customers.customers.view.notes.customer-not-notified', ['date' => core()->formatDate($note->created_at, 'm/d/Y h:i:s A')])
                @endif
            </p>
        </div>
    @endforeach
