...
<x-admin::form.control-group>
    ...
    @else
        ...
        {{-- Multi- Country select Vue component --}}
        @elseif ($field['type'] == 'multi-country')
        $selectedOption = core()->getConfigData($nameKey, $currentChannel->code, $currentLocale->code);
            <v-multi-country ref="multicountryRef">
                    ...
                    <x-admin::form.control-group class="flex">
                        ...
                        >
                            @foreach (core()->countries() as $country)
                                <option value="{{ $country->code }}"  {{ in_array($key, explode(',', $selectedOption)) ? 'selected' : ''}}>{{ $country->name }}</option>
                            @endforeach
                        </x-admin::form.control-group.control>
                    </x-admin::form.control-group>
                </template>
...
@if ($field['type'] == 'multi-country')
    @pushOnce('scripts')
        <script type="text/x-template" id="v-multi-country-template">
            <div>
                <slot
        ...
        <script type="module">
            app.component('v-multi-country', {
                template: '#v-multi-country-template',
